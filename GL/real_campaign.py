#!/usr/bin/env python3
"""Run and aggregate the ten-repetition full-Covtype Group Lasso campaign."""
...
def parse_args(argv: Optional[Sequence[str]] = None):
    ...
    parser.add_argument(
        "--results-dir",
        default="results/covtype",
    )
    parser.add_argument(
        "--seeds", default=",".join(str(seed) for seed in range(2026, 2036))
    )
    parser.add_argument("--datasets", default="covtype")
...
def run_campaign(root: Path, datasets: Iterable[str], seeds: Iterable[int], time_limit: float, resume: bool):
    env = os.environ.copy()
    env["LIBSVMDATA_HOME"] = str(HERE / "data" / "libsvmdata")
...
def aggregate_runs(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for (dataset, method), block in frame.groupby(["dataset", "method"], sort=False):
        ...
        for metric in metrics:
            ...
            row[metric + "_std"] = float(values.std(ddof=1))
...
def format_mean_std(mean: float, std: float) -> str:
    return "%.6g (%.3g)" % (mean, std)


def write_report(root: Path, summary: pd.DataFrame, seeds: Sequence[int]):
    lines = [
        "# Group Lasso full-Covtype ten-repetition results",
        "",
        "Protocol: seeds %s; all ten methods use the same split within each repetition. "
        ...
        "no public author repository was found. The LDPM variants use the "
        "full-state stopping rule $R_k \\le 10^{-5}$ "
        ...
        "Parentheses contain the sample standard deviation across ten runs."
        % ", ".join(map(str, seeds)),
        "",
    ]
    for dataset in [item for item in ("covtype",) if item in set(summary["dataset"])]:
        lines.extend(
            [
                "## %s" % dataset,
                "",
                "| Method | Converged | Time mean (SD) | Validation error mean (SD) | Test error mean (SD) |",
                "|---|---:|---:|---:|---:|",
            ]
        )
        for _, row in summary[summary["dataset"] == dataset].iterrows():
            lines.append(
                ...
                % (
                    ...
                    row["converged_runs"],
                    row["runs"],
                    format_mean_std(row["time_mean"], row["time_std"]),
                    format_mean_std(row["val_loss_mean"], row["val_loss_std"]),
                    format_mean_std(row["test_loss_mean"], row["test_loss_std"]),
                )
            )
        lines.extend(
            [
                ...
                "",
                "| Method | Validation miscl. mean (SD) | Test miscl. mean (SD) |",
                "|---|---:|---:|",
            ]
        )
        for _, row in summary[summary["dataset"] == dataset].iterrows():
            lines.append(
                ...
                % (
                    ...
                    format_mean_std(
                        ...
                        row["val_misclassification_std"],
                    ),
                    format_mean_std(
                        row["test_misclassification_mean"],
                        row["test_misclassification_std"],
                    ),
                )
            )
        lines.append("")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if len(seeds) != 10 or len(set(seeds)) != 10:
        raise ValueError("the reporting protocol requires exactly ten distinct seeds")
    if datasets != ["covtype"]:
        raise ValueError("the manuscript real-data campaign uses only full Covtype")
    ...
    protocol = {
        ...
        "stopping_rule": "full-state relative change <= 1e-5",
        ...
        "dispersion": "sample standard deviation across ten runs (ddof=1)",
        "reported_error": "half mean squared prediction error",
        "supplementary_metrics": "validation/test misclassification are retained in all_runs.csv and summary_mean_std.csv",
    }
