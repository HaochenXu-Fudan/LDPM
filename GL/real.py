#!/usr/bin/env python3
"""Run one manuscript Group Lasso experiment on the full Covtype data.

The default split is stratified 72/8/20.  Only the first ten quantitative
features are standardized with training statistics; the 44 indicator columns
remain in their original 0/1 representation.
"""
...
# data imports libsvmdata at module load, so point that import at the
# repository-local immutable dataset cache before importing it.
...
def preprocess_covtype(x_train, x_validation, x_test):
    """Apply the manuscript train-only preprocessing to full Covtype."""

    arrays = [
        value.toarray().astype(float, copy=False)
        if sparse.issparse(value)
        else np.asarray(value, dtype=float).copy()
        for value in (x_train, x_validation, x_test)
    ]
    binary_values = np.unique(arrays[0][:, 10:])
    if not np.all(np.isin(binary_values, (0.0, 1.0))):
        raise ValueError("Covtype indicator columns must remain binary")
    mean = np.mean(arrays[0][:, :10], axis=0)
    std = np.std(arrays[0][:, :10], axis=0)
    std = np.where(std > 1e-12, std, 1.0)
    for array in arrays:
        array[:, :10] = (array[:, :10] - mean) / std
...
def prepare_data(args: argparse.Namespace):
    ...
    if official_test_key is not None:
        ...
        split_rule = "stratified 72/8/20 train/validation/test split"

    if canonical == "covtype":
        processed, quantitative_mean, quantitative_std = preprocess_covtype(
            x_train, x_validation, x_test
        )
        x_train, x_validation, x_test = processed
        preprocessing = (
            "features 1--10 standardized by training mean/std; "
            "features 11--54 retained as 0/1"
        )
    else:
        scale = train_rms_scale(x_train)
        x_train = scaled_dense(x_train, scale)
        x_validation = scaled_dense(x_validation, scale)
        x_test = scaled_dense(x_test, scale)
        quantitative_mean = np.zeros(x_train.shape[1], dtype=float)
        quantitative_std = scale
        preprocessing = "train-only column RMS scaling without centering"
    data = Data()
    data.X_train = np.asarray(x_train, dtype=float)
    data.X_validate = np.asarray(x_validation, dtype=float)
    data.X_test = np.asarray(x_test, dtype=float)
    ...
    metadata = {
        ...
        "label_rule": "Cover Type 2 -> +1; all other cover types -> -1",
        "preprocessing": preprocessing,
        "quantitative_mean": quantitative_mean.tolist(),
        "quantitative_std": quantitative_std.tolist(),
...
def run_method(
    ...
    row = {
        ...
        "r_stat": (
            float(last["r_stat"])
            if "r_stat" in last and np.isfinite(float(last["r_stat"]))
            else (
                float(last["x_lambda_stop"])
                if np.isfinite(float(last["x_lambda_stop"]))
                else None
            )
        ),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--dataset", choices=sorted(DATASET_KEYS), default="covtype")
    parser.add_argument("--seed", type=int, default=2026)
    parser.add_argument("--tol", type=float, choices=[1e-4, 1e-5], default=1e-5)
    parser.add_argument(
        "--methods",
        default="grid,random,tpe,igjo,vf-idca,ldmma,meha,agils,ldpm,ldpm-capped",
    )
    ...
    parser.add_argument("--results-dir", default="results/covtype")
    ...
    parser.add_argument("--capped-step-size", type=float, default=0.01)
    parser.add_argument("--capped-beta0", type=float, default=0.5)
    parser.add_argument("--capped-beta-power", type=float, default=0.3)
    parser.add_argument("--beta-max-capped", type=float, default=5.0)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print(
        "Stopping: full-state relative change <= %.1e; step=%.3g; time limit=%.1fs"
    ...
    metadata["solver_config"] = {
        ...
        "paper29_protocol": args.paper29_protocol,
        "stop_metric": (
            "baselines retain their method-specific rules; LDPM uses "
            "full-state relative change <= tolerance"
        ),
