#!/usr/bin/env python3
"""Run one manuscript synthetic Sparse Group Lasso experiment.

The default contract uses p=600, ten disjoint groups of size 60,
400/400/400 samples, SNR 2, and a global entrywise l1 penalty.
"""
...
def prepare_data(
    ...
    if group_count <= 0 or p % group_count:
        raise ValueError("group_count must be positive and divide p exactly")
    if p // 5 < 10:
    ...
    settings = SimpleNamespace(
        ...
        num_test=n_test,
        num_features=p,
        num_experiment_groups=group_count,
        num_true_groups=5,
        dataset="synthetic_group_lasso_p%d" % p,
    )
    problem_name = (
        "sparse Group Lasso (%d group-l2 penalties plus one l1 penalty)"
        % group_count
        if sparse_group
        else "pure Group Lasso (%d group-l2 penalties; no l1 regularizer)"
        % group_count
    )
    metadata = {
        "data_protocol": (
        ...
        "group_size": p // group_count,
        "truth_group_count": 5,
        "truth_macro_group_size": group_size,
...
def experiment_regularizers(
    p: int, sparse_group: bool, group_count: int = 5
) -> List[dict]:
    regularizers = group_regularizers(p, group_count)
    if sparse_group:
...
def run_method(data_info, args, method: str, run_dir: Path) -> Dict[str, object]:
    ...
    regularizers = experiment_regularizers(p, args.sparse_group, args.group_count)
    ...
    try:
        with time_limit(args.time_limit, label):
            if method in ldpm_methods:
                ...
                initial_radius = None
                if method == "meha" and args.meha_initial_lambda_vector:
                    initial_lambda = _parse_lambda_values(
                        args.meha_initial_lambda_vector
                    )
                    if initial_lambda.size != parameter_count:
                        raise ValueError(
                            "--meha-initial-lambda-vector needs %d values, got %d"
                            % (parameter_count, initial_lambda.size)
                        )
                    if np.any(initial_lambda < 0.0):
                        raise ValueError(
                            "--meha-initial-lambda-vector must be nonnegative"
                        )
                if method == "vf-idca":
                ...
                baseline_setting = {
                    ...
                    "paper_relative_tol": (
                        args.meha_paper_relative_tol
                        if args.meha_paper_relative_tol is not None
                        else 0.005 / p
                    ),
                    "meha_feasibility_tol": args.meha_feasibility_tol,
                    "agils_epsilon": 1e-6,
                    "agils_penalty0": 6.0,
                ...
                meha_initial_lower_iterations = None
                if method == "meha" and args.meha_initial_coef == "lower":
                    initial_lambda_array = np.asarray(
                        initial_lambda, dtype=float
                    )
                    if initial_lambda_array.ndim == 0:
                        initial_lambda_array = np.full(
                            parameter_count, float(initial_lambda_array)
                        )
                    initial_coef, meha_initial_lower_iterations = problem.lower_solve(
                        initial_lambda_array,
                        x0=common_initial_coef,
                        max_iter=args.lower_max_iter,
                        tol=args.lower_tol,
                    )
                    baseline_setting["initial_coef"] = initial_coef
                agils_beta = 1.0 / (
                    problem.val_lipschitz / 6.0 + problem.train_lipschitz + 0.1
                )
                meha_alpha = (
                    float(args.meha_alpha)
                    if args.meha_alpha is not None
                    else (1.0 / 1.1) / args.meha_alpha_divisor
                )
                meha_beta = (
                    float(args.meha_beta)
                    if args.meha_beta is not None
                    else agils_beta / args.meha_beta_divisor
                )
                meha_eta = (
                    float(args.meha_eta)
                    if args.meha_eta is not None
                    else (1.0 / (problem.train_lipschitz + p))
                    / args.meha_eta_divisor
                )
                baseline_setting.update(
                    meha_alpha=meha_alpha,
                    meha_beta=meha_beta,
                    meha_eta=meha_eta,
                )
                if method == "meha":
                    algorithm_config = {
                        ...
                        "eta": meha_eta,
                        "gamma": 1.0 / p,
                        "c0": float(args.meha_c0),
                        "c_power": float(args.meha_c_power),
                        "initial_coef": str(args.meha_initial_coef),
                        "initial_lower_iterations": meha_initial_lower_iterations,
                        "initial_lambda": np.asarray(
                            initial_lambda, dtype=float
                        ).reshape(-1).tolist()
                        if np.asarray(initial_lambda).ndim
                        else [float(initial_lambda)] * parameter_count,
                        "lambda_floor": 1e-8,
                        "lambda_ceiling": (
                            float(args.meha_lambda_ceiling)
                            if np.isfinite(args.meha_lambda_ceiling)
                            else None
                        ),
                        "paper_relative_tol": float(
                            args.meha_paper_relative_tol
                            if args.meha_paper_relative_tol is not None
                            else 0.005 / p
                        ),
                        "feasibility_tol": args.meha_feasibility_tol,
                    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--p", type=int, default=600)
    parser.add_argument("--n-train", type=int, default=400)
    parser.add_argument("--n-validate", type=int, default=400)
    ...
    parser.add_argument("--results-dir", default="results/sparse_group_lasso")
    ...
    parser.add_argument("--cs-step-size", type=float, default=0.1)
    parser.add_argument("--cs-capped-step-size", type=float, default=0.1)
    parser.add_argument("--cs-gamma", type=float, default=1.0)
    parser.add_argument("--cs-beta0", type=float, default=0.03)
    parser.add_argument("--cs-beta-power", type=float, default=1.2)
    ...
    parser.add_argument("--beta-max-capped", type=float, default=775.0)
    ...
    parser.add_argument(
        "--meha-initial-lambda-vector",
        help="optional comma- or semicolon-separated MEHA lambda0 vector",
    )
    parser.add_argument(
        "--meha-initial-coef",
        choices=["ones", "lower"],
        default="ones",
        help="initialize MEHA at ones or at x*(lambda0)",
    )
    parser.add_argument("--meha-alpha", type=float)
    parser.add_argument("--meha-beta", type=float)
    parser.add_argument("--meha-eta", type=float)
    parser.add_argument("--meha-c0", type=float, default=20.0)
    parser.add_argument("--meha-c-power", type=float, default=0.1)
    parser.add_argument("--meha-paper-relative-tol", type=float)
    parser.add_argument("--meha-feasibility-tol", type=float)
    parser.add_argument("--meha-lambda-ceiling", type=float, default=10.0)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print(
        "Data: %s p=%d groups=%d train/val/test=%d/%d/%d SNR=%.12g"
        % (
    ...
    for method in methods:
        ...
        row["snr"] = float(args.snr)
        row["group_count"] = int(args.group_count)
        row["group_size"] = int(args.p // args.group_count)
        row["n_train"] = int(args.n_train)
        row["n_validate"] = int(args.n_validate)
        row["n_test"] = int(args.n_test)
    ...
    metadata["protocol"] = {
        ...
        "search": (
            "Grid %dx%d tied-group/l1 weights; Random and TPE %d %d-dimensional trials"
            % (
                args.grid_points,
                args.grid_points,
                args.search_budget,
            ...
            if args.sparse_group
            else "Grid %d common weights; Random and TPE %d %d-dimensional trials"
            % (args.grid_points, args.search_budget, args.group_count)
        ),
        "initialization": {
            "LDPM": {
            ...
            "MEHA": {
                "coefficient": args.meha_initial_coef,
                "lambda": (
                    _parse_lambda_values(
                        args.meha_initial_lambda_vector
                    ).tolist()
                    if args.meha_initial_lambda_vector
                    else [float(args.initial_lambda)]
                ),
            },
            "AGILS_IGJO": "coefficient=ones, lambda=ones",
