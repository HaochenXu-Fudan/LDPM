#!/usr/bin/env python3
"""Run the ten-repetition manuscript Sparse Group Lasso campaign.

The contract is p=600, ten groups of size 60, 400/400/400 samples,
SNR 2, half-MSE losses, and the manuscript continuation parameters.
"""
...
BASELINE_METHODS = METHOD_KEYS[:8]
LDPM_METHODS = METHOD_KEYS[-2:]
METHOD_LABELS = {
    "grid": "Grid",
    "random": "Random",
    "tpe": "TPE",
...
EXPECTED_LDPM_INITIAL_LAMBDA = [0.01] * 10 + [0.75]
...
def common_command(args, seed: int, methods: Iterable[str]) -> List[str]:
    driver = Path(__file__).resolve().parent / "experiment.py"
    return [
        sys.executable,
        str(driver),
        "--p",
    ...
    if group_name == "baselines":
        ...
        command.extend(
            [
                ...
                "0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.75",
                "--ldpm-init-dual",
                "kkt",
                "--init-max-iter",
                "5000",
...
def write_report(
    ...
    lines = [
        ...
        "",
        "Data: n_train=n_validation=n_test=400, m=600, ten equal groups, "
        "ten group-l2 hyperparameters plus one l1 hyperparameter, SNR=2. "
        "Independent seeds: %s." % ", ".join(str(seed) for seed in seeds),
        "",
        "Entries are mean (sample standard deviation) over ten attempted runs. "
        ...
        "",
        "| Method | Finite/10 | Status | Time (s) | Val. err. | Test err. | Test err. infeas. | Feasibility |",
        "|---|---:|---|---:|---:|---:|---:|---:|",
    ]
    ...
    lines.extend(
        ...
        "",
        "Time is aggregated over all ten attempts. Error and feasibility "
        "statistics exclude failed, timed-out, and nonfinite attempts, but retain "
        "finite max-iteration endpoints. All twenty final LDPM-CS/LDPM-CS-C runs "
        ...
        "it is not author source code. AGILS-A and AGILS-R are not run.",
            ...
            "compatible coefficient/dual start with lambda0=(0.01 repeated "
            "for ten groups, 0.75 for the l1 penalty), selected validation-only on independent seeds "
            "2024 and 2025. Both stop when max(complete-state residual, consensus "
            "residual) is <=1e-5. LDPM-CS-C reached beta_max=775 "
            "in %d/10 runs." % capped_reached,
            "",
            "## LDPM per-run convergence audit",
            "",
            "| Method | Seed | Status | Iter. | r_stat | r_cons | Feasibility |",
            "|---|---:|---|---:|---:|---:|---:|",
        ]
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--results-dir",
        default="results/sparse_group_lasso",
    )
