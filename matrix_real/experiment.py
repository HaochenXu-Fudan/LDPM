...
The contract follows ``INFORMS-IJOC-Template.tex``: one task-wise split, all
139 tasks, and LDPM-CS / capped LDPM-CS-C only.  Optimization is test-blind.  A
...
class LDPMConfig:
    ...
    consensus_tol: Optional[float] = 1e-5
    convergence_window: int = 1
    max_time: Optional[float] = 1800.0
...
def run_ldpm(
    ...
    for iteration in range(1, config.max_iter + 1):
        ...
        state_change_sq = (
            float(np.dot(z - old_z, z - old_z))
            + float(np.dot(xi - old_xi, xi - old_xi))
            + float(np.sum((copies - old_copies) ** 2))
            + float(np.sum((multipliers - old_multipliers) ** 2))
        )
        old_state_norm_sq = (
            float(np.dot(old_z, old_z))
            + float(np.dot(old_xi, old_xi))
            + float(np.sum(old_copies**2))
            + float(np.sum(old_multipliers**2))
        )
        change = math.sqrt(state_change_sq) / max(1.0, math.sqrt(old_state_norm_sq))
        next_step = min(
        ...
        consensus_change = max(
            ...
            max(float(np.linalg.norm(copy)) for copy in copies),
        )
        consensus_ok = (
        ...
        if converged:
            status = "success"
            message = "full-state relative change and consensus residual met the tolerance"
            break
...
def _plot_heatmaps(output: Path, matrices: Dict[str, Array], task_means: Array) -> None:
    # Some execution environments have a read-only home-level matplotlib cache.
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).with_name("results") / "school" / "seed2026",
    )
    ...
    parser.add_argument("--beta-power", type=float, default=0.2)
    ...
    parser.add_argument("--consensus-tol", type=float, default=1e-5)
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    protocol = {
        ...
        "stopping": "full-state relative change and normalized consensus residual <= 1e-5",
